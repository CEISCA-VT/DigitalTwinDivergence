...
\section{Provenance Repairs and Pending Network Emulation}
\label{app:pending}
...
The causal-derivative study is complete and shows that the service-level remediability structure is reasonably robust to eliminating centered-gradient lookahead, despite substantially worse trajectory fidelity after retraining. The doubly nested bank and its corrected comparator analysis are also complete. The remaining planned computational experiment is Linux \texttt{netem}, which will replace software-generated arrival patterns with measured OS-level delay and loss traces while retaining the same precomputed-state scope.
