...
\label{sec:magnet}
The MAGNET experiment is included to test a different question from i2Nav, TerraSentia, or UGV01: does the proposed fidelity \emph{structure} remain meaningful when the physical state, prediction horizon, and application domain change completely? The public INL dataset was produced during the March~30,~2022 MAGNET single-heat-pipe demonstration, in which the DT ran alongside the physical test article and forecast future thermal behavior \cite{wilsdon2023magnet,inlmagnetdata}. We use the released physical experiment and released ML forecast as fixed evidence; no MAGNET model is trained or tuned in this work.

Let $T_p(t)$ denote measured heater temperature and let $\widehat{T}_T(t+h\mid t)$ denote the DT forecast, issued at time $t$, for forecast horizon $h$. A thermal-domain component discrepancy is
\begin{equation}
D_T(t,h)=\left|T_p(t+h)-\widehat{T}_T(t+h\mid t)\right|,
\end{equation}
and the corresponding signed residual is
\begin{equation}
r_T(t,h)=T_p(t+h)-\widehat{T}_T(t+h\mid t).
\end{equation}
This preserves the same evaluation logic without forcing robot-specific metrics onto the thermal system. Forecast horizon supplies the finite-horizon axis; signed residual summaries expose persistent over- or under-prediction; and p95/max summaries expose tail behavior. Unlike mobile-robot ATE, there is no reason to invent a trajectory-global position metric for this domain. The framework transfers by preserving the \emph{roles} of component error, horizon, persistence, and tails while allowing their physical realization to change.

\begin{table}[t]
\caption{MAGNET cross-domain fidelity-audit provenance. The window counts describe eligibility and the dependence-reduced check; they are not treated as 111 independent experiments.}
\label{tab:magnet}
\centering
\footnotesize
...
Audit item & Value \\
\midrule
Physical system & Single heat pipe in MAGNET \\
Physical response & Heater temperature \\
Published forecast horizon & up to 10 min \\
Candidate forecast windows & 111 \\
Strict eligible windows & 97 \\
Non-overlapping sensitivity windows & 23 \\
Model retraining/tuning here & none \\
\bottomrule
\end{tabular}
\end{table}

The resulting audit passed the predeclared evidence checks used to decide whether the case was strong enough for main-text inclusion. More importantly, it changes the nature of the generality claim. TFP is no longer supported only by planar robot trajectories: the same architecture-independent fidelity organization can be instantiated on a thermal forecasting DT whose state variable and time scale are different. We therefore describe MAGNET as evidence of \emph{cross-domain structural transfer}, not as proof that one numerical fidelity profile or one set of thresholds transfers unchanged between robots and thermal systems.

\begin{figure}[t]
\centering
\safegraphic{figures/magnet_cross_domain_transfer.png}
\caption{Cross-domain TFP instantiation. The mobile-robot study uses pose/motion discrepancies and second-scale relative-motion horizons, whereas MAGNET uses physical-versus-forecast heater-temperature discrepancy and minute-scale forecast horizons. The common object is the fidelity structure---component error, horizon dependence, signed persistence, tails, and statistically defensible aggregation---not the robot-specific metric names.}
...
MAGNET provides a useful falsification check on the concern that TFP is only ATE/RPE repackaged as DT terminology. No robot pose, heading, wheel odometry, or SE(2) relative transform exists in the heat-pipe audit. Instead, the physical/virtual pair is measured versus forecast heater temperature, and fidelity evolves over a 10-min forecast horizon. The same organizing principles remain usable because they are defined at the level of a physical quantity and its twin counterpart rather than at the level of a particular estimator. This is empirical evidence for structural portability across two substantially different CPS domains. It is still a limited result: one public heat-pipe experiment and one primary thermal response do not establish universal DT fidelity across energy, structural, manufacturing, electrical, or biological systems.
