...
const char* ap_ssid = "Shreyas_UGV";
const char* ap_password = "DigitalTwin";
...
bool createWifiConfigFileByStatus() {
	if (WIFI_MODE_ON_BOOT != 0 || WIFI_MODE_ON_BOOT != 255){
		wifiDoc.clear();
...
bool wifiModeAP(const char* input_ssid, const char* input_password) {
  ...
  applyStableWiFiSettings();

  // channel 6, visible SSID, max 1 client
  bool ap_ok = WiFi.softAP(input_ssid, input_password, 6, 0, 1);
  ...
  if (InfoPrint == 1) {
    ...
    Serial.print("AP OK: ");
    Serial.println(ap_ok ? "true" : "false");
  ...
  jsonInfoHttp["ap_ok"] = ap_ok;
  jsonInfoHttp["ap_ssid"] = ap_ssid;
  jsonInfoHttp["ap_password"] = ap_password;
  jsonInfoHttp["ap_ip"] = localIP.toString();
...
bool wifiModeAPSTA(const char* input_ap_ssid, const char* input_ap_password, const char* input_sta_ssid, const char* input_sta_password) {
	...
	// channel 6, visible SSID, max 1 client
	bool ap_ok = WiFi.softAP(input_ap_ssid, input_ap_password, 6, 0, 1);
	if (InfoPrint == 1) {
